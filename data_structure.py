...
# 询问用户是否爬取到足够多的文本
sufficient_texts = input("是否爬取到足够多的文本？爬取到足够多的文本将使用你自己的数据，否则将使用样例数据。请输入 'Y' 或 'N': ").strip()

# 根据用户输入选择文件夹路径
if sufficient_texts == 'Y':
    folder_path = os.path.join(current_dir, 'data_posts', 'original_thuhole_texts')
elif sufficient_texts == 'N':
    folder_path = os.path.join(current_dir, 'data_posts', 'original_thuhole_texts_example')
else:
